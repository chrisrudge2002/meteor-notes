...
if (Meteor.isServer) {
	describe('notes', function () {
		...
		it('should not remove note if _id not specified', function () {
			expect(() => { Meteor.server.method_handlers['notes.remove'].apply({ userId: noteOne.userId }); }).toThrow();
		});

		it('should update note if authenticated', function () {
		...
		it('should not update note if not authenticated', function () {
			expect(() => { Meteor.server.method_handlers['notes.update'].apply({}, []); }).toThrow();
		});

		it('should not update note if _id not specified', function () {
			expect(() => { Meteor.server.method_handlers['notes.update'].apply({ userId: noteOne.userId }, []); }).toThrow();
		});
